...
contract Voting is IForwarder, AragonApp {
    ...
    modifier voteExists(uint256 _voteId) { // @note check is voteId from 1 -> mistake could be
    ...
    */
    function initialize(MiniMeToken _token, 
        ...
        require(_minBalance >= _minBalanceLowerLimit && _minBalance <= _minBalanceUpperLimit); // can be 0
        require(_minTime >= _minTimeLowerLimit && _minTime <= _minTimeUpperLimit); // can be 0

        token = _token;
        supportRequiredPct = _supportRequiredPct;
        minAcceptQuorumPct = _minAcceptQuorumPct;
        voteTime = _voteTime; // can be 0
    ...
    */
    function vote(uint256 _voteData, bool _supports, bool _executesIfDecided) external voteExists(_decodeData(_voteData, 0, MAX_UINT_128)) {
        ...
        _vote(voteId, yeaPct, nayPct, msg.sender, _executesIfDecided); // can vote for 0 yeas and no
    }
    ...
    */
    function _vote(uint256 _voteId, uint256 _yeaPct, uint256 _nayPct, address _voter, bool _executesIfDecided) internal { // @audit можно ли поставить yea + nay != 100?
        ...
        uint256 voterStake = uint256(2).mul(balance).mul(vote_.startDate.add(voteTime).sub(getTimestamp64())).div(voteTime); // доля ост время * баланс * 2
        if(voterStake > balance) { // если больше или равно половине периода то голосует всем балансом походу из-за *2
            voterStake = balance;
        }
    ...
    */
    function _isValuePct(uint256 _value, uint256 _total, uint256 _pct) internal pure returns (bool) {
        ...
        return computedPct > _pct; // @audti почему не больше равно??
    }
