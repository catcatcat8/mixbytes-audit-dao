...
contract LA is ERC20 {
    ...
    uint256 constant private INITIAL_RATE = 100638977635782747603833865 / YEAR; // @note rate v sekyndy ~3.191 * 10**18
    uint256 constant private RATE_REDUCTION_TIME = YEAR; // @note год, через год rate видимо сокращается
    // 1.252
    uint256 constant private RATE_REDUCTION_COEFFICIENT = 1252 * 1e15; // 1.252 * 10**18
    ...
    constructor() ERC20('Litra Token', 'LA') {
        ...
        admin = _msgSender();
        startEpochTime = block.timestamp + INFLATION_DELAY - RATE_REDUCTION_TIME; // 364 дня назад
        miningEpoch = -1;
        rate = 0;
    ...
    function updateMiningParamters() external {
        require(block.timestamp >= startEpochTime + RATE_REDUCTION_TIME, 'Not time'); // через день после конструктора можно вызвать
     ...
     */
    function mintableInTimeframe(uint256 start, uint256 end) external view returns(uint256) { // start 3 day, end 375 day po staromy rate??
